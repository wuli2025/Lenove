...
const S = {
  ...
  screen: 'today',
  repTab: 'daily',
  // 编辑区里还没按下「把今天存起来」的东西，只留在这台设备上，不参与云同步
  draft: { mood: '', text: '', media: [] },
  noteTarget: null,
  customCat: 'emotion',
  rec: null,
...
/* ───────── 编辑区暂存（草稿） ───────── */
// 相册/拍摄/录音挑进来的东西先落在这里，按下「把今天存起来」才并进当天的记录
const DRAFT_DAY = '__draft';
function loadDraft() {
  const d = store.get('draft', null) || {};
  return { mood: d.mood || '', text: d.text || '', media: Array.isArray(d.media) ? d.media : [] };
}
function persistDraft() { store.setQuiet('draft', S.draft); }   // 本机私事，不上云
function clearDraft() { S.draft = { mood: '', text: '', media: [] }; persistDraft(); }
function draftEmpty() { return !S.draft.mood && !S.draft.text.trim() && !S.draft.media.length; }
/** 查看器/缩略图统一从这里取某一天的媒体，DRAFT_DAY 代表还没存起来的那批 */
function mediaOf(day) {
  return day === DRAFT_DAY ? S.draft.media : ((S.journal[day] || {}).media || []);
}
...
function renderMoodRow() {
  // 编辑区只认草稿：存过的内容都在下面「写过的日子」里，这里永远是「正要写的那一条」
  const sel = S.draft.mood;
...
function saveMood() {
  ...
  const add  = S.draft.media || [];
  const cur  = S.journal[S.today] || {};
  if (!S.draft.mood && !text && !add.length) { toast('选个心情，或者写两句、放张照片都行'); return; }

  const curMedia = cur.media || [];
  const first = !cur.mood && !cur.text && !curMedia.length;
  // 同一天再存一次就往后接，不覆盖前面写过的
  const merged = [cur.text, text].map(s => (s || '').trim()).filter(Boolean).join('\n\n');
  const media = [...curMedia, ...add];
  S.journal[S.today] = {
    mood: S.draft.mood || cur.mood || '😊',
    text: merged,
    ts: Date.now(),
    ...(media.length ? { media } : {}),
  };
  ...
  clearDraft();                    // 上面的心情、文字、照片一起清空，东西已经进下面的记录了
  if (first) {
    S.day.points += 5; S.meta.totalPoints += 5;
    persistDay(); persistMeta(); renderProgress(); renderMe();
    toast('今天存好了，+5 颗糖 🍬', 2300);
  } else {
    toast(add.length ? `存好了，${tallyText(mediaTally(add), true) || add.length + ' 个'}都收进下面的记录了 💛` : '存好了，都收进下面的记录了 💛', 2400);
  }
...
const MEDIA = {
  ...
  async add(fileList) {
    ...
    renderMediaStrip();
    toast(`挑好了 ${ok} 个 📷 按下「把今天存起来」才会收进下面的记录`, 3400);
    if (window.HappySync && HappySync.on) this.flush();
  ...
  async addRecorded(blob, dur, mime) {
    ...
    renderMediaStrip();
    toast('录好了 🎙 按下「把今天存起来」才会收进下面的记录', 3400);
    if (window.HappySync && HappySync.on) this.flush();
  },

  /** 先挂在编辑区的草稿上，按下「把今天存起来」才进当天的记录 */
  attach(item) {
    S.draft.media = [...(S.draft.media || []), item];
    persistDraft();
  },

  /** 还没存起来就想撤掉的那一个 */
  async removeDraft(id) {
    S.draft.media = (S.draft.media || []).filter(m => m.id !== id);
    persistDraft();
    await this.purge(id);
  },
...
function renderMediaStrip() {
  ...
  if (!box) return;
  const list = S.draft.media || [];
  box.hidden = !list.length;
  box.innerHTML = list.map(m => `<div class="mslot">
      ${mediaTile(m, `data-day="${DRAFT_DAY}"`)}
      <button class="mslot-del" data-mdel="${m.id}" aria-label="删掉这个">×</button>
    </div>`).join('');
  hydrateMedia(box);
  const tip = $('#strip-tip');
  if (tip) {
    tip.hidden = !list.length;
    tip.textContent = `这 ${list.length} 个还没存起来，按下面的按钮才收进记录`;
  }
}

/* ── 点开看大图 / 放视频 ── */
function openViewer(day, id) {
  const list = mediaOf(day);
...
function renderViewer() {
  ...
  const m = list[idx];
  const when = day === DRAFT_DAY ? '还没存起来' : (d => `${d.getMonth() + 1}月${d.getDate()}日`)(parseKey(day));
  const bits = [when, `${idx + 1}/${list.length}`];
...
async function saveViewerFile() {
  ...
  a.href = u;
  a.download = `幸福小事-${v.day === DRAFT_DAY ? S.today : v.day}-${m.id.slice(0, 6)}.${ext}`;
  a.click();
  toast('存下来了 ⬇');
}
...
function reloadState() {
  ...
  S.journal  = store.get('journal', {});
  S.draft    = loadDraft();          // 草稿只在本机，同步不会动它
  S.mdel     = store.get('mdel', {});
  S.custom   = store.get('custom', []);
  MEDIA.reap();
...
function bind() {
  ...
  $('#mood-row').onclick = e => {
    ...
    // 再点一下同一个就是取消
    S.draft.mood = S.draft.mood === b.dataset.mood ? '' : b.dataset.mood;
    persistDraft();
    $$('.mood-opt').forEach(x => x.classList.toggle('is-on', x === b && !!S.draft.mood));
  };
  $('#mood-input').oninput = () => {
    updateNoteCount();
    S.draft.text = $('#mood-input').value;
  ...
  $('#media-strip').onclick = async e => {
    ...
    if (del) {
      if (!confirm('这个还没存起来，扔掉吗？')) return;
      await MEDIA.removeDraft(del.dataset.mdel);
      renderMediaStrip();
      return;
    }
    const tile = e.target.closest('.mtile');
  ...
  $('#mv-del').onclick = async () => {
    ...
    const m = mediaOf(v.day)[v.i];
    if (!m) return;
    const draft = v.day === DRAFT_DAY;
    if (!confirm(draft ? '这个还没存起来，扔掉吗？' : '删掉这个吗？云端那份也会一起删。')) return;
    if (draft) await MEDIA.removeDraft(m.id);
    else await MEDIA.remove(v.day, m.id);
