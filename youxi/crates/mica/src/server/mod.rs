...
pub fn router(state: AppState) -> axum::Router {
    ...
    axum::Router::new()
        ...
        .route(
            "/v1/tasks/{id}",
            get(routes::get_task).merge(patch(routes::patch_task)),
        )
        ...
        .route(
            "/v1/providers",
            get(routes::list_providers).merge(post(routes::add_provider)),
        )
        .route(
            "/v1/providers/{id}/activate",
            post(routes::activate_provider),
        )
