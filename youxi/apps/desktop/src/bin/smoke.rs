...
fn sample() -> Requirement {
    Requirement {
        ...
        sections: vec!["它是谁".into(), "老照片".into(), "那些小事".into(), "最后一年".into()],
...
#[tokio::main]
async fn main() {
    let runs: usize = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(1);
    ...
    for i in 1..=runs {
        ...
        match r {
            Ok(o) => {
                let bytes = std::fs::metadata(dir.join("index.html")).map(|m| m.len()).unwrap_or(0);
                println!(
                    "\n     ✅ {:.1}s · {} token · {} 字节{}",
                    o.elapsed_ms as f64 / 1000.0,
                    o.tokens,
                    bytes,
                    if o.degraded { format!(" · 降级：{}", o.degrade_reason) } else { String::new() }
                );
    ...
    println!("耗时  最快 {:.1}s   中位 {:.1}s   P90 {:.1}s   最慢 {:.1}s",
        times[0] as f64 / 1000.0, pick(0.5), pick(0.9), times[times.len() - 1] as f64 / 1000.0);
    println!("token 平均 {}   最高 {}",
        toks.iter().sum::<u64>() / toks.len() as u64, toks.iter().max().unwrap());
    println!("降级 {degraded}/{}", oks.len());

    // 十分钟是对观众的硬承诺，超了就该红灯
    let over = times.iter().filter(|&&t| t > cfg.hard_deadline_secs * 1000).count();
    if over > 0 {
...
fn short(p: &Progress) -> String {
    ...
    format!("{}·{:.0}s", &s[..s.len().min(4)], p.elapsed_ms as f64 / 1000.0)
}
